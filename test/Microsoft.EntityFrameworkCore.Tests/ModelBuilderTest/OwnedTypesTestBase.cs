    ...
    public abstract partial class ModelBuilderTest
    {
        public abstract class OwnedTypesTestBase : ModelBuilderTestBase
        {
            ...
            public virtual void Can_declare_owned_type()
            {
                ...
                var entityBuilder = modelBuilder.Entity<Customer>().Owns(c => c.Details);
            ...
            public virtual void Can_configure_owned_type()
            {
                ...
                modelBuilder.Entity<Customer>().Owns(c => c.Details)
                    .Property<int>("foo");
            ...
            [Fact]
            public virtual void Can_configure_ownership()
            {
                var modelBuilder = CreateModelBuilder();
                var model = modelBuilder.Model;

                modelBuilder.Entity<Customer>().Owns(c => c.Details, r => r.HasAnnotation("foo", "bar"));

                var ownership = model.FindEntityType(typeof(Customer)).FindNavigation(nameof(Customer.Details)).ForeignKey;
                Assert.Equal("bar", ownership.FindAnnotation("foo").Value);
            }
        }
    }
}
